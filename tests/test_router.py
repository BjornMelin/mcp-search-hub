...
def create_test_result(title: str = "Test Result", url: str = "http://test.com") -> SearchResult:
...
def create_test_response(results: list[SearchResult] = None, query: str = "test") -> SearchResponse:
...
class TestProviderMetrics:
    ...
    def test_record_success(self):
        ...
        assert metrics.error_count == 0
        assert abs(metrics.avg_response_time - 0.2) < 0.001  # Allow for floating point precision
        assert metrics.success_rate == 1.0
        assert metrics.is_healthy is True
...
class TestSearchRouter:
    ...
    async def test_route_and_execute_success(self, router):
        ...
        mock_result1 = create_test_response([create_test_result("Result 1", "http://test1.com")])
        mock_result2 = create_test_response([create_test_result("Result 2", "http://test2.com")])
    ...
    async def test_execute_with_circuit_breaker_success(self, router):
        ...
        mock_result = create_test_response([create_test_result("Test", "http://test.com")])
        provider = router.providers["provider1"]
        provider.search.return_value = mock_result
        
        query = SearchQuery(query="test")
        
        result = await router._execute_with_circuit_breaker("provider1", provider, query)
...
class TestProviderSelectionStrategies:
    ...
    def router_with_metrics(self):
        ...
        router = SearchRouter(providers=providers, max_concurrent=3, default_timeout=30.0)
...
class TestCircuitBreakerIntegration:
    ...
    async def test_circuit_breaker_opens_after_failures(self, router_with_failures):
        ...
        router_with_failures.providers["failing_provider"].search.side_effect = Exception("API Error")
        router_with_failures.providers["reliable_provider"].search.return_value = create_test_response([], "test")
    ...
    async def test_healthy_provider_continues_working(self, router_with_failures):
        ...
        router_with_failures.providers["failing_provider"].search.side_effect = Exception("API Error")
        router_with_failures.providers["reliable_provider"].search.return_value = create_test_response(
        ...
        results = await router_with_failures.route_and_execute(query, features, max_providers=2)
