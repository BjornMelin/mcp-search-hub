...
async def test_end_to_end_search_basic(mock_server, mock_context):
    ...
    # Execute search
    response = await mock_server.search_with_routing(query, "test-request-id", mock_context)
...
async def test_end_to_end_search_with_caching(mock_server, mock_context):
    ...
    response1 = await mock_server.search_with_routing(query, "test-request-id-1", mock_context)
    ...
    response2 = await mock_server.search_with_routing(query, "test-request-id-2", mock_context)
...
async def test_end_to_end_different_content_types(mock_server, mock_context):
    ...
    for query_text, expected_content_type in queries:
        ...
        response = await mock_server.search_with_routing(query, f"test-{expected_content_type}", mock_context)
...
async def test_unified_router_strategy_selection(mock_server, mock_context):
    ...
    for case in test_cases:
        ...
        with patch.object(mock_server.router, 'select_providers') as mock_select:
            with patch.object(mock_server.router, '_determine_strategy', wraps=mock_server.router._determine_strategy) as mock_determine:
                ...
                # Execute search
                await mock_server.search_with_routing(query, f"test-{case['expected_strategy']}", mock_context)
