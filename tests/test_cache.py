...
def create_test_combined_response(results: list[SearchResult] = None, query: str = "test") -> CombinedSearchResponse:
...
class TestSearchCache:
    ...
    def test_initialization_success(self):
        ...
        with patch('mcp_search_hub.utils.cache.REDIS_AVAILABLE', True):
            with patch('mcp_search_hub.utils.cache.redis.from_url') as mock_redis_from_url:
                ...
                mock_redis_from_url.assert_called_once_with("redis://localhost:6379", decode_responses=False)
    ...
    def test_initialization_redis_connection_error(self):
        ...
        with patch('mcp_search_hub.utils.cache.REDIS_AVAILABLE', True):
            with patch('mcp_search_hub.utils.cache.redis.from_url', side_effect=Exception("Connection failed")):
                cache = SearchCache()
                assert cache.redis_client is None
    ...
    async def test_set_success(self, mock_redis, sample_query, sample_response):
        ...
        cache = SearchCache(default_ttl=300, ttl_jitter=0)  # No jitter for predictable testing
        cache.redis_client = mock_redis
...
class TestCacheIntegration:
    ...
    def test_cache_performance_characteristics(self):
        ...
        queries = [
            SearchQuery(query=f"test query {i}")
            for i in range(1000)
        ]
