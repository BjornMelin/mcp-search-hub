...
class TestModelTrainer:
    ...
    def test_create_training_dataset_fallback(self):
        ...
        with mock.patch(
            "mcp_search_hub.query_routing.ml_models.model_trainer.USE_ML", False
        ):
            trainer = ModelTrainer()
            examples = [
    ...
    def test_create_training_examples_from_queries(self):
        ...
        assert any(
            ex.text1 == "how to make pasta"
            and ex.text2 == "how to cook spaghetti"
            and ex.score == 1.0
            for ex in examples
        )
        assert any(
            ex.text1 == "best smartphone 2025"
            and ex.text2 == "top mobile phones this year"
            and ex.score == 1.0
            for ex in examples
        )
        ...
        assert any(
            ex.text1 == "machine learning"
            and ex.text2 == "healthy recipes"
            and ex.score == 0.0
            for ex in examples
        )
        assert any(
            ex.text1 == "climate change"
            and ex.text2 == "fantasy novels"
            and ex.score == 0.0
            for ex in examples
        )
        ...
        assert any(
            ex.text1 == "python tutorial"
            and ex.text2 == "learn python programming"
            and ex.score == 0.9
            for ex in examples
        )
        assert any(
            ex.text1 == "hiking trails"
            and ex.text2 == "mountain hiking paths"
            and ex.score == 0.7
            for ex in examples
        )
