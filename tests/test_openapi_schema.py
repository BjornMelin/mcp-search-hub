...
def test_custom_openapi_generation(mock_app):
    ...
    assert schema["openapi"] in [
        "3.1.0",
        "3.0.2",
    ]  # Both are valid, depends on FastAPI version
...
def test_export_openapi_endpoint(mock_app):
    ...
    async def export_openapi():
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": "attachment; filename=mcp-search-hub-openapi.json"
            },
        )
    ...
    assert (
        ...
        == "attachment; filename=mcp-search-hub-openapi.json"
    )
