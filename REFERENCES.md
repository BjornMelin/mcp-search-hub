# References

This document provides links to official documentation for all technologies and services used in the MCP Search Hub project.

## Core Technologies

### FastMCP

- [FastMCP Documentation](https://github.com/anthropics/FastMCP)
- [Model Context Protocol (MCP) Specification](https://github.com/anthropics/anthropic-tools/blob/main/protocol.md)
- [FastMCP Python Client](https://github.com/anthropics/FastMCP)

### Python Libraries

- [Pydantic 2.x Documentation](https://docs.pydantic.dev/latest/)
- [HTTPX Documentation](https://www.python-httpx.org/)
- [Python Asyncio Documentation](https://docs.python.org/3/library/asyncio.html)
- [Python Type Hints](https://docs.python.org/3/library/typing.html)
- [Python Environment Variables](https://docs.python.org/3/library/os.html#os.environ)

## Search Providers

### Linkup

- [Linkup API Documentation](https://docs.linkup.com/reference/getting-started-1)
- [Linkup Search API Reference](https://docs.linkup.com/reference/search-1)
- [Linkup API Authentication](https://docs.linkup.com/reference/authentication)
- [Linkup Pricing](https://www.linkup.com/pricing)
- [Linkup MCP Server (Python)](https://github.com/LinkupPlatform/python-mcp-server)
- [Linkup MCP Documentation](https://docs.linkup.so/pages/integrations/mcp/mcp)

### Exa

- [Exa API Documentation](https://docs.exa.ai/reference/getting-started)
- [Exa Search API Reference](https://docs.exa.ai/reference/search)
- [Exa Content Types](https://docs.exa.ai/reference/content-types)
- [Exa API Keys](https://dashboard.exa.ai/api-keys)
- [Exa Pricing](https://exa.ai/pricing)
- [Exa MCP Server](https://github.com/exa-labs/exa-mcp-server)
- [Exa MCP Documentation](https://docs.exa.ai/examples/exa-mcp)

### Perplexity

- [Perplexity API Documentation](https://docs.perplexity.ai/)
- [Perplexity API Reference](https://docs.perplexity.ai/reference/post_chat_completions)
- [Perplexity Search Capabilities](https://docs.perplexity.ai/docs/search-capabilities)
- [Perplexity API Keys](https://www.perplexity.ai/settings/api)
- [Perplexity Pricing](https://www.perplexity.ai/pricing)
- [Perplexity MCP Server](https://github.com/ppl-ai/modelcontextprotocol)
- [Perplexity MCP Documentation](https://docs.perplexity.ai/guides/mcp-server)

### Tavily

- [Tavily API Documentation](https://docs.tavily.com/)
- [Tavily Search API Reference](https://docs.tavily.com/reference/search-1)
- [Tavily API Authentication](https://docs.tavily.com/docs/authentication)
- [Tavily Pricing](https://tavily.com/pricing)
- [Tavily MCP Server](https://github.com/tavily-ai/tavily-mcp)
- [Tavily MCP Documentation](https://docs.tavily.com/documentation/mcp)

### Firecrawl

- [Firecrawl API Documentation](https://docs.firecrawl.dev/)
- [Firecrawl Search API Reference](https://docs.firecrawl.dev/reference/search)
- [Firecrawl Authentication](https://docs.firecrawl.dev/docs/authentication)
- [Firecrawl Deep Research](https://docs.firecrawl.dev/docs/deep-research)
- [Firecrawl Pricing](https://firecrawl.dev/pricing)
- [Firecrawl MCP Server](https://github.com/mendableai/firecrawl-mcp-server)

## MCP Server Integration

### Server Implementation

- [FastMCP Server Guide](https://github.com/anthropics/FastMCP#server)
- [FastMCP Tool Registration](https://github.com/anthropics/FastMCP#registering-tools)
- [FastMCP Transport Options](https://github.com/anthropics/FastMCP#transports)

### MCP Clients

- [Claude Desktop MCP Integration](https://docs.anthropic.com/claude/docs/tool-use)
- [Claude Code MCP Configuration](https://docs.anthropic.com/en/docs/claude-code)
- [Anthropic Python SDK Tool Use](https://docs.anthropic.com/claude/docs/tool-use-with-claude)
- [VS Code Claude Extension](https://github.com/anthropics/claude-vscode)
- [Cursor AI Integration](https://cursor.sh/docs/anthropic-claude)
- [Windsurf Integration](https://windsurf.io/)

## Deployment

- [Docker Documentation](https://docs.docker.com/)
- [Docker Compose Documentation](https://docs.docker.com/compose/)
- [Python Virtual Environments](https://docs.python.org/3/library/venv.html)
- [Environment Variables with Python-Dotenv](https://github.com/theskumar/python-dotenv)

## Development Tools

- [Ruff Documentation](https://github.com/astral-sh/ruff)
- [Pytest Documentation](https://docs.pytest.org/)
- [UV Python Package Installer](https://github.com/astral-sh/uv)

## Benchmarks

- [SimpleQA Benchmark](https://github.com/tatsu-lab/simple_qa)
- [MT-Bench](https://github.com/lm-sys/FastChat/tree/main/fastchat/llm_judge)
- [MMLU Benchmark](https://github.com/hendrycks/test)
