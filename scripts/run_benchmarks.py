...
async def benchmark_parallel_strategy(provider_counts=[2, 5, 10], result_counts=[10, 20, 50]):
    ...
    for provider_count in provider_counts:
        ...
        print(f"  Successful providers: {sum(1 for r in results.values() if r.success)}/{provider_count}")
        response_times = [p.response_time_ms for p in providers.values()]
        print(f"  Provider response times: min={min(response_times):.0f}ms, max={max(response_times):.0f}ms")
...
async def benchmark_cascade_strategy(provider_counts=[2, 5, 10], fail_rates=[0, 0.5]):
    ...
    for provider_count in provider_counts:
        for fail_rate in fail_rates:
            ...
            # Create query
            query = SearchQuery(query=f"benchmark cascade query")
            features = QueryFeatures(
                length=10,
                word_count=3,
                content_type="general",
            ...
            print(f"  Successful providers: {sum(1 for r in results.values() if r.success)}/{len(results)}")
            
            # Calculate how long before we found a successful provider
            fail_count = sum(1 for r in results.values() if not r.success)
            response_times = [p.response_time_ms for p in list(providers.values())[:fail_count+1]]
...
async def benchmark_result_merger(result_counts=[50, 100, 500, 1000]):
    ...
    for result_count in result_counts:
        ...
        print(f"  Deduplication rate: {(result_count - len(merged_results)) / result_count * 100:.1f}%")
...
async def benchmark_throughput(concurrent_queries=[5, 10, 20, 50]):
    ...
    for concurrency in concurrent_queries:
        ...
        for results in provider_results:
            merger_tasks.append(
                asyncio.create_task(
                    asyncio.to_thread(merger.merge_results, results)
                )
            )
        ...
        print(f"  Results per query: {sum(len(r) for r in merged_results) / concurrency:.1f}")


async def main():
    """Run all benchmarks and output results."""
    parser = argparse.ArgumentParser(description="Run MCP Search Hub performance benchmarks")
    parser.add_argument("--parallel", action="store_true", help="Run parallel strategy benchmarks")
    parser.add_argument("--cascade", action="store_true", help="Run cascade strategy benchmarks")
    parser.add_argument("--merger", action="store_true", help="Run result merger benchmarks")
    ...
    run_all = args.all or not (args.parallel or args.cascade or args.merger or args.throughput)
    
    # Print header
    print(f"MCP Search Hub Performance Benchmarks")
