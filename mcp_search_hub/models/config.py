...
class ProviderConfig(ComponentConfig):
    ...
    rate_limit_per_day: Optional[int] = Field(
        None, description="Maximum requests per day"
    )
...
class RouterConfig(ComponentConfig):
    ...
    base_timeout_ms: int = Field(
        10000, description="Base timeout in milliseconds"
    )
    min_timeout_ms: int = Field(
        3000, description="Minimum timeout in milliseconds"
    )
    max_timeout_ms: int = Field(
        30000, description="Maximum timeout in milliseconds"
    )
...
class PerformanceTrackerConfig(ComponentConfig):
    ...
    history_size: int = Field(
        100, description="Number of queries to keep in history"
    )
...
class RateLimitConfig(BaseModel):
    ...
    cooldown_period: int = Field(
        5, description="Seconds to wait when rate limited"
    )
...
class BudgetConfig(BaseModel):
    ...
    default_query_budget: float = Field(
        0.02, description="Max cost per query"
    )
    daily_budget: float = Field(
        10.00, description="Max daily cost"
    )
    monthly_budget: float = Field(
        150.00, description="Max monthly cost"
    )
    enforce_budget: bool = Field(
        True, description="Whether to enforce budgets"
    )
