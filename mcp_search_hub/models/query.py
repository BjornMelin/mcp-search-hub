...
class SearchQuery(BaseModel):
    ...
    advanced: bool = Field(False, description="Whether to use advanced search capabilities")
    max_results: int = Field(10, description="Maximum number of results to return", ge=1, le=100)
    content_type: Optional[str] = Field(None, description="Optional explicit content type hint")
    providers: Optional[List[str]] = Field(None, description="Optional explicit provider selection")
    budget: Optional[float] = Field(None, description="Optional budget constraint in USD")
    timeout_ms: int = Field(5000, description="Timeout in milliseconds", ge=1000, le=30000)
...
class QueryFeatures(BaseModel):
    ...
    contains_question: bool = Field(..., description="Whether the query contains a question")
