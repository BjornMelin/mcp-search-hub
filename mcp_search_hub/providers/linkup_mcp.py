        ...
        try:
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                if hasattr(text_content, "text"):
                    ...
                    search_data = (
                        eval(text_content.text)
                        if text_content.text.startswith("[")
                    ...
                    if isinstance(search_data, list):
                        for idx, item in enumerate(search_data[: query.max_results]):
                            if isinstance(item, dict):
                                search_result = SearchResult(
                                    ...
                                    source="linkup",
                                    score=1.0
                                    - (idx * 0.1),  # Decreasing score by position
                        ...
                        search_result = SearchResult(
                            ...
                            url="https://linkup.so",
                            snippet=str(search_data)[:200] + "..."
                            if len(str(search_data)) > 200
                            else str(search_data),
                            source="linkup",
                            score=1.0,
