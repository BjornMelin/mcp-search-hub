...
class SearchRouter:
    ...
    def _select_providers(
        ...
        if strategy == "best_performance":
            ...
            healthy_providers.sort(key=lambda name: (
                self.metrics[name].avg_response_time / max(0.1, self.metrics[name].success_rate)
            ))
        elif strategy == "power_of_two":
            # Power of two random selection for load balancing
            if len(healthy_providers) >= 2:
                candidates = random.sample(healthy_providers, min(2, len(healthy_providers)))
                healthy_providers = sorted(
                    candidates,
                    key=lambda name: self.metrics[name].avg_response_time
                )
    ...
    async def check_health(self) -> tuple[HealthStatus, str]:
        ...
        healthy_count = sum(
            1 for name in self.providers 
            if self.providers[name].enabled and self.metrics[name].is_healthy
        )
        
        if healthy_count == 0:
            return HealthStatus.UNHEALTHY, "No healthy providers"
        elif healthy_count < len(enabled_providers) / 2:
            return HealthStatus.DEGRADED, f"Only {healthy_count}/{len(enabled_providers)} providers healthy"
        else:
            return HealthStatus.HEALTHY, f"{healthy_count}/{len(enabled_providers)} providers healthy"
