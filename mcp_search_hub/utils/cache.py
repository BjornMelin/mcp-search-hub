...
class TieredCache:
    ...
    async def get_async(self, key: str) -> Optional[CombinedSearchResponse]:
        """
    ...
    async def set_async(self, key: str, result: CombinedSearchResponse) -> None:
        ...
        if self.redis_enabled and self.async_redis:
            try:
                serialized = self._serialize(result)
                await self.async_redis.setex(self._redis_key(key), self.redis_ttl, serialized)
                logger.debug(f"Set key in Redis cache: {key}")
            except Exception as e:
                logger.warning(f"Failed to set data in Redis: {e}")
            ...
            if self.redis_enabled and self.redis:
                try:
                    ...
                    if redis_keys:
                        ...
                        logger.debug(f"Cleared {len(redis_keys)} keys matching pattern from Redis: {pattern}")
    ...
    def _deserialize(self, data: bytes) -> CombinedSearchResponse:
        ...
        """
        try:
            return CombinedSearchResponse.model_validate(json.loads(data.decode('utf-8')))
        except Exception as e:
