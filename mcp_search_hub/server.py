...
from .models.results import SearchResult, SearchResponse, CombinedSearchResponse
...
class SearchServer:
    ...
    def _register_tools(self):
        ...
        async def search(query: SearchQuery, ctx: Context) -> CombinedSearchResponse:
            ...
            if cached_result and not query.providers:  # Skip cache if explicit providers
            ...
            for provider_name in providers_to_use:
                if provider_name in self.providers:
                    ...
                    provider_tasks[provider_name] = asyncio.create_task(provider.search(query))
            ...
            try:
                ...
                for provider_name, task in provider_tasks.items():
                    if task in done and not task.exception():
                        ...
                        if task.exception():
                            await ctx.error(f"Provider {provider_name} error: {str(task.exception())}")
                        else:
